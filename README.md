...
## What it's safe to claim in the report

- Learned rank prediction can drastically reduce **logical comparisons per
  lookup** on smooth (linear, uniform) distributions in this prototype.
- A simple two-stage piecewise linear model recovers some of the win on
  irregular (lognormal) data, but not on a worst-case piecewise-mixed dataset
  with a single linear router.
- A learned-only index without fallback is **brittle**: under any distribution
  drift it can return wrong results (found_rate → 0) while still appearing
  fast in step count.
- A hybrid policy with a **bisect fallback** preserves correctness (found_rate
  matches bisect in every phase tested here) and prevents catastrophic
  worst-case behavior.
- A simple **rolling fallback-rate detector** plus retraining is sufficient
  to bring the model back online when the new distribution is fittable, and
  the give-up rule prevents pathological retrain churn when it isn't.
- The `confidence_threshold` exposes a **non-monotonic tradeoff** between
  fallback rate and search cost; the right setting is data-dependent and
  closely tied to the model's training error.

## What to avoid claiming

- **No claim about absolute speed vs a production index (B+-tree, ART,
  RocksDB, etc.).** The prototype is pure Python with `bisect.bisect_left`
  and small numpy fits; comparing its nanoseconds to a C++ B+-tree is
  meaningless and the project is explicitly not about that.
- **Caution with the latency plots.** Median-ns numbers in this repo are
  dominated by Python interpreter and `time.perf_counter_ns` overhead. The
  ordering is informative but the magnitudes shouldn't be quoted as evidence
  for or against learned indexes in general.
- **No claim about generalization to real datasets** beyond what the four
  synthetic distributions show. The drift schedule is constructed; it
  demonstrates the *mechanism*, not what production drift looks like.
- **No claim that this is RMI / ALEX / PGM.** The model and structure here
  are deliberately simpler — one linear or one two-level piecewise model on
  a static sorted array — so the experiments are explainable end-to-end.
- **Don't read "fallback rate" as "bug rate."** A fallback that finds the
  correct answer is a feature working as intended; the relevant cost metric
  is `mean_steps`, not fallback rate by itself.

## Reproducibility notes

- Every randomized step uses an explicit seed (`numpy.random.default_rng`).
  The CSV numbers above were produced with the script defaults.
- `gc.disable()` is called around the timing loops to keep variance lower,
  then re-enabled. This is a small, ordinary precaution; it does not change
  the logical metrics.
- All datasets are generated at runtime; no downloads, no large files in the
  repo.
